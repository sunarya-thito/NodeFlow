...
public class ShiftLeftMath extends AbstractNodeProvider {
    public ShiftLeftMath(NodeProviderCategory category) {
        super("java#shiftleft", "Shift Left Binary", category);
    ...
    public CompileSession createNewSession() {
        return new CompileSession() {
            ...
            protected void handleCompile() {
                getHandler().setReference(getNode().getParameter(2), Java.Math.shl(
        ...
        return new NodeCompileSession() {
            ...
            public void handleCompile(NodeCompileHandler handler) {
                if (isParameterUsed(node.getParameter(2))) {
                    handler.putReference(node.getParameter(2), Java.Math.shl(
