...
public class JREBundle extends JavaBundleImpl {

    public JREBundle(BundleProperties bundleProperties, BundleManager bundleManager) {
        super(bundleProperties, bundleManager);

        //
        initializeDefaultJavaEvents();
    }

    private void initializeDefaultJavaEvents() {
        NodeProviderCategory providerCategory = new SimpleNodeProviderCategory("Conditional Nodes", "Condition", null);
        providerCategory.getProviders().add(new IfElseBlock(providerCategory));
        providerCategory.getProviders().add(new NegateLogic(providerCategory));
        providerCategory.getProviders().add(new GreaterCompare(providerCategory));
        providerCategory.getProviders().add(new GreaterEqualsCompare(providerCategory));
        providerCategory.getProviders().add(new LessCompare(providerCategory));
        providerCategory.getProviders().add(new LessEqualsCompare(providerCategory));
        providerCategory.getProviders().add(new NullCompare(providerCategory));
        providerCategory.getProviders().add(new NotNullCompare(providerCategory));
        providerCategory.getProviders().add(new EqualsCompare(providerCategory, Object.class));
        ModuleManagerImpl.getInstance().registerCategory(providerCategory);
        providerCategory = new SimpleNodeProviderCategory("Math Nodes", "Math", null);
        providerCategory.getProviders().add(new UnsignedShiftRightMath(providerCategory));
        providerCategory.getProviders().add(new ShiftRightMath(providerCategory));
        providerCategory.getProviders().add(new ShiftLeftMath(providerCategory));
        ModuleManagerImpl.getInstance().registerCategory(providerCategory);
        providerCategory = new SimpleNodeProviderCategory("Looping Nodes", "Looping", null);
        providerCategory.getProviders().add(new WhileProvider(providerCategory));
        providerCategory.getProviders().add(new ForEachProvider(providerCategory));
        providerCategory.getProviders().add(new ForEachArrayProvider(providerCategory));
        providerCategory.getProviders().add(new LoopTimesProvider(providerCategory));
        ModuleManagerImpl.getInstance().registerCategory(providerCategory);
        providerCategory = new SimpleNodeProviderCategory("Value Nodes", "Value", null);
        providerCategory.getProviders().add(new NullProvider(providerCategory));
        ModuleManagerImpl.getInstance().registerCategory(providerCategory);
        ModuleManagerImpl.getInstance().registerCategory(new ArrayProviderCategory());
        ModuleManagerImpl.getInstance().registerCategory(new JavaNodeProviderCategory(Utility.class));
    }
    ...
    public Set<String> getAvailableClasses() {
        return new Set<String>() {
            ...
            public boolean contains(Object o) {
                return Arrays.binarySearch(CLASSES, o) >= 0;
            }
            ...
            public <T> T[] toArray(T[] a) {
                return Arrays.asList(CLASSES).toArray(a);
            }
            ...
            public boolean containsAll(Collection<?> c) {
                return Arrays.asList(CLASSES).containsAll(c);
            }
