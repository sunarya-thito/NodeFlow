...
//        private void runTimelineY(double newTransY, double newHeight) {
//            if(selectedTabLine.getTranslateY() == newTransY
//                    && scale.getY() == newHeight) return;
//
//            double tempScaleY = 0;
//            double tempHeight = 0;
//            final double lineHeight = selectedTabLine.prefHeight(-1);
//
//            if ((isAnimating())) {
//                timeline.stop();
//                tempScaleY = scale.getY();
//                if (rotate.getAngle() != 0) {
//                    rotate.setAngle(0);
//                    tempHeight = tempScaleY * lineHeight;
//                    selectedTabLine.setTranslateY(selectedTabLine.getTranslateY() - tempHeight);
//                }
//            }
//
//            final double oldScaleY = scale.getY();
//            final double oldHeight = lineHeight * oldScaleY;
//            final double oldTransY = selectedTabLine.getTranslateY();
//            final double newScaleY = (newHeight * oldScaleY) / oldHeight;
//            // keep track of the original offset
//            selectedTabLineOffset = newTransY;
//            // add offset to the computed translation
//            newTransY = newTransY + offsetStart * direction;
//            final double transDiff = newTransY - oldTransY;
//
//
//            double midScaleY = tempScaleY != 0 ?
//                    tempScaleY : ((Math.abs(transDiff)/translateScaleFactor + oldHeight) * oldScaleY) / oldHeight;
//
//            if(midScaleY > Math.abs(transDiff) + newHeight){
//                midScaleY = Math.abs(transDiff) + newHeight;
//            }
//            if (transDiff < 0) {
...
//                    new KeyFrame(
//                            Duration.seconds(.12),
//                            new KeyValue(scale.yProperty(), midScaleY, Interpolator.EASE_BOTH),
//                            new KeyValue(selectedTabLine.translateYProperty(), selectedTabLine.getTranslateY(), Interpolator.EASE_BOTH)),
//                    new KeyFrame(
//                            Duration.seconds(.24),
...
//        }
        ...
        private void runTimelineX(double newTransX, double newWidth) {
        ...
        private void updateScrollOffset(double newOffset) {
            ...
            for (Node node : headersRegion.getChildren()) {
                if (node instanceof TabHeaderContainer) {
                    double tabHeaderPrefWidth = snapSize(isHorizontal() ? node.prefWidth(-1) : node.prefHeight(-1));
            ...
            if (actualOffset != scrollOffset) {
                ...
                if (!isAnimating()) {
                    if (isHorizontal()) {
                        ...
                        selectedTabLine.setTranslateY(selectedTabLineOffset + scrollOffset * direction);
                    }
                }
            }
        }
        ...
        private void updateSelectionLine(boolean animate) {
            ...
            if(selectedTabWidth > 0){
                if (animate) {
                    runTimelineX(selectedTabOffset, selectedTabWidth);
                }else{
        ...
        public HeaderControl(ArrowPosition pos) {
            ...
            container.setOnMousePressed(press -> {
                ...
                if (isHorizontal()) {
                    ...
                    double offset = isLeftArrow ? header.scrollOffset + header.headersRegion.getHeight() : header.scrollOffset - header.headersRegion
                            .getHeight();
                    arrowAnimation = new Timeline(new KeyFrame(Duration.seconds(1),
                            new KeyValue(offsetProperty, offset, Interpolator.LINEAR)));
                    arrowAnimation.play();
