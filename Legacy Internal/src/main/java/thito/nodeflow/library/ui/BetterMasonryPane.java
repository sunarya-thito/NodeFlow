 ...
 * @since 2016-05-24
 */
public class BetterMasonryPane extends Pane {
    ...
    private static double roundSize(double size, double maxSize, int total) {
        double smallSize = maxSize / total;
        double roundedSize = smallSize;
        while (roundedSize < size) {
            roundedSize += smallSize;
        }
        return roundedSize;
    }
...
//        for (int i = 0; i < managedChildren.size(); i++) {
//            Region child = managedChildren.get(i);
//            if (!(child instanceof GridPane)) {
//                child.setPrefWidth(roundSize(child.getWidth(), width, col));
//                child.setPrefHeight(roundSize(child.getHeight(), height, row));
//            }
//        }

        // get bounding boxes layout
        newBoxes = layoutMode.get().fillGrid(this, matrix, managedChildren,
        ...
        for (int i = 0; i < managedChildren.size() && i < newBoxes.size(); i++) {
            ...
            if (!(child instanceof GridPane)) {
                ...
                if (animationMap == null) {
                    ...
                    child.setMinSize(blockWidth, blockHeight);
    ...
    public static abstract class LayoutMode {
        ...
        protected abstract List<BoundingBox> fillGrid(BetterMasonryPane pane, int[][] matrix, List<Region> children, double cellWidth, double cellHeight, int limitRow, int limitCol, double gutterX, double gutterY);
         ...
         * @return
         */
        protected BoundingBox getFreeArea(BetterMasonryPane pane, int[][] matrix, int x, int y, Region block, double cellWidth, double cellHeight, int limitRow, int limitCol, double gutterX, double gutterY) {
...
//            }
    ...
    private static class MasonryLayout extends LayoutMode {
        @Override
        public List<BoundingBox> fillGrid(BetterMasonryPane pane, int[][] matrix, List<Region> children, double cellWidth, double cellHeight, int limitRow, int limitCol, double gutterX, double gutterY) {
            int row = matrix.length;
            if (row <= 0) {
                return null;
            }
            int col = matrix[0].length;
            List<BoundingBox> boxes = new ArrayList<>(children.size());
            ...
            for (int b = 0; b < children.size(); b++) {
                ...
                for (int i = 0; i < row; i++) {
                    ...
                    for (int j = 0; j < col; j++) {
                        ...
                        BoundingBox box = getFreeArea(pane, matrix,
