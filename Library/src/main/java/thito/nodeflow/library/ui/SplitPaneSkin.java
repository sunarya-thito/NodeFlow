 ...
 *
 * NODEFLOW: Patched the divider so it becomes overlay
 * The divider is placed on top of the splitting line
 * instead of placed in between. This removes the annoying
 * divider gap for transparent divider.
 *
 * @see SplitPane
 * @since 9
 */
...
//                        double newPos = getAbsoluteDividerPos(previousDivider) + dividerWidth;
                        double newPos = getAbsoluteDividerPos(previousDivider);
                        // Middle panels
...
//                    space = (horizontal ? w : h) - (previousDivider != null ? getAbsoluteDividerPos(previousDivider) + dividerWidth : 0);
                    space = (horizontal ? w : h) - (previousDivider != null ? getAbsoluteDividerPos(previousDivider): 0);
                }
...
//                    total += (dividerWidth * contentDividers.size());
    ...
    @Override protected double computePrefWidth(double height, double topInset, double rightInset, double bottomInset, double leftInset) {
        ...
        // NODEFLOW
        // Content Divider is overlay and should not affect the SplitPane
//        for (ContentDivider d: contentDividers) {
...
//            double max = previousDividerPos == 0 ? maxLeft : previousDividerPos + maxLeft;

            double min = nextDividerPos - minRight - dividerWidth;
//            double min = nextDividerPos - minRight;
...
//                double rightMax = nextDividerPos - maxRight;
...
//                startX += getLeft(d).getArea() + (i == 0 ? 0 : dividerWidth);
                startX += getLeft(d).getArea() - dividerWidth / 2d + (i == 0 ? 0 : dividerWidth);
            } else {
                // NODEFLOW
//                startY += getLeft(d).getArea() + (i == 0 ? 0 : dividerWidth);
                startY += getLeft(d).getArea() - dividerWidth / 2d + (i == 0 ? 0 : dividerWidth);
            }
            d.setX(startX);
            d.setY(startY);
