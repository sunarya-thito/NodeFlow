...
//    private Map<String, Integer> fieldThreadMap = new HashMap<>();
    ...
    private int getThreadIdFromDescriptor(String descriptor) {
        return switch (descriptor) {
            case "Lthito/nodeflow/annotation/BGThread;" -> 1;
            case "Lthito/nodeflow/annotation/IOThread;" -> 2;
            case "Lthito/nodeflow/annotation/UIThread;" -> 3;
            default -> 0;
        };
    }
    @Override
                            ...
                            private void writeThreadCheckInvocation(int threadID) {
                                switch (threadID) {
                                    case 1 -> // Background Thread
                                            super.visitMethodInsn(Opcodes.INVOKESTATIC, "thito/nodeflow/task/TaskThread", "BG", "()Lthito/nodeflow/task/TaskThread;", true);
                                    case 2 -> // IO Thread
                                            super.visitMethodInsn(Opcodes.INVOKESTATIC, "thito/nodeflow/task/TaskThread", "IO", "()Lthito/nodeflow/task/TaskThread;", true);
                                    case 3 -> // UI Thread
                                            super.visitMethodInsn(Opcodes.INVOKESTATIC, "thito/nodeflow/task/TaskThread", "UI", "()Lthito/nodeflow/task/TaskThread;", true);
                                }
                                if (threadID != 0) {
...
//                File outDebug = new File("cache", name + ".class");
//                try {
//                    Files.write(outDebug.toPath(), classBytes);
//                } catch (Throwable t) {
//                    t.printStackTrace();
//                }
